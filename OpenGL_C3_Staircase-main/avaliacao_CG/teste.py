...
import numpy as np
from rasterizacao import bresenham_line, setup_line  # Importa a função de rasterização
...
class App:
    def __init__(self):
        ...
        self.view_pos = np.array([0.0, 0.0, 3.0], dtype=np.float32)
        self.light_color = np.array([1.0, 1.0, 1.0], dtype=np.float32)
        self.object_color = np.array([1.0, 0.5, 0.31], dtype=np.float32)
        
        # Configuração da linha rasterizada
        self.line_points = bresenham_line(100, 100, 500, 300)  # Exemplo de linha
        self.line_vao, self.line_vbo = setup_line(self.line_points)
    ...
    def glMultMatrix(self, matrix_ptr):
        ...
        result_matrix = np.dot(matrix.reshape(4, 4), current_matrix)
    ...
    def run(self):
        ...
        while not glfw.window_should_close(self.window):
            ...
            # Define as uniformes de iluminação
            glUniform3fv(glGetUniformLocation(self.shader, "lightPos"), 1, self.light_pos)
            glUniform3fv(glGetUniformLocation(self.shader, "viewPos"), 1, self.view_pos)
            glUniform3fv(glGetUniformLocation(self.shader, "lightColor"), 1, self.light_color)
            glUniform3fv(glGetUniformLocation(self.shader, "objectColor"), 1, self.object_color)
            ...
            # Aplica a translação usando a função glMultMatrix
            ...
            # Desenha a linha rasterizada
            glBindVertexArray(self.line_vao)
            glDrawArrays(GL_POINTS, 0, len(self.line_points))
